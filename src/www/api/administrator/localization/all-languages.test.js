...
describe('/api/administrator/localization/all-languages', () => {
  const cachedResponses = {}
  const cachedLanguages = []
  before(async () => {
    const allLanguages = require('../../../../../index.js').languageList
    for (const language of allLanguages) {
      cachedLanguages.push(language.languageid)
    }
    await TestHelper.setupBeforeEach()
    const administrator = await TestHelper.createUser()
    global.delayDiskWrites = true
    const req1 = TestHelper.createRequest('/api/administrator/localization/all-languages?offset=1')
    req1.account = administrator.account
    req1.session = administrator.session
    cachedResponses.offset = await req1.get()
    const req2 = TestHelper.createRequest('/api/administrator/localization/all-languages?limit=1')
    req2.account = administrator.account
    req2.session = administrator.session
    cachedResponses.limit = await req2.get()
    const req3 = TestHelper.createRequest('/api/administrator/localization/all-languages?all=true')
    req3.account = administrator.account
    req3.session = administrator.session
    cachedResponses.all = await req3.get()
    const req4 = TestHelper.createRequest('/api/administrator/localization/all-languages')
